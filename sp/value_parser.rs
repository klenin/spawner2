use instance::{
    EnvType, EnvVar, EnvVars, PipeKind, RedirectFlags, SpawnerOptions, StderrRedirectList,
...
fn parse_stdio_redirect(
    ...
    if !s.starts_with("*") {
        // file
        Ok(Some(parse_file_redirect(
            s,
            SpawnerOptions::DEFAULT_FILE_FLAGS,
        )))
    } else if s.starts_with("*:") {
        // *: or *:file or *:n.stdio
        if len == 2 {
            list.default_flags = SpawnerOptions::DEFAULT_FILE_FLAGS;
            Ok(None)
        } else {
        ...
        Ok(Some(parse_pipe_redirect(
            &s[1..],
            SpawnerOptions::DEFAULT_PIPE_FLAGS,
        )?))
    }
}
